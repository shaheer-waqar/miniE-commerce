    ...
    <div className='py-20 bg-zinc-200' >
        <div className='w-full flex px-5 justify-between  gap-5 flex-col md:flex-row '>
            ...
            <div className='flex-col gap-2 '>
              ...
              <select onChange={(e)=>setOption(e.target.value)} value={option} className='bg-transparent text-sm  py-1 outline-none w-full md:w-auto'>
                <option value="all Products">All Products</option>
                {categories.map((e,i)=><option key={e} value={e}>{e}</option>)}
              </select>
            </div>
        </div>
