...
function SingleProduct({ addToCart }) {
  ...
  useEffect(() => {
    if (singleProduct && realted.length > 0) {
      let filterd = realted.filter(
        (e, i) =>
          e.category == singleProduct.category && e.id != singleProduct.id
      );
  ...
  return (
    <div className="bg-zinc-200">
      <div className="pt-10">
        {singleProduct ? (
          <div className="w-full px-2  md:h-auto py-5">
            <div
              className="max-w-[900px] shadow-md m-auto flex flex-col
              gap-2 mt-10 md:flex-row"
            >
      ...
      {filteredData.length > 0 ? (
        <div>
          <h1 className="px-5 text-lg font-semibold">You Might Also Like</h1>

          <div className="grid grid-cols-1 gap-5 px-4 pt-2 pb-2 md:grid-cols-3 lg:grid-cols-4">
